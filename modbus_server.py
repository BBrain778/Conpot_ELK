...
class ModbusServer(modbus.Server):
    ...
    def _configure_slaves(self, template):
        dom = etree.parse(template)
        slaves = dom.xpath("//modbus/slaves/*")  # 從模板讀取從站設定
        try:
            for s in slaves:
                slave_id = int(s.attrib["id"])
                # 呼叫 SlaveBase 的 add_slave 方法
                slave = self._databank.add_slave(slave_id)  # 假設 databank 是 SlaveBase 類別
            ...
            logger.error(f"Error while configuring slaves: {e}")
        ...
        try:
            while True:
                ...
                response, logdata = self._databank.handle_request(
                    query, request, self.mode, address[0]
                )
